...
#import "SelectAutoMobileBandModelCellView.h"
#import "SelectDateView.h"
#import "AutomobileBrandModelViewController.h"
@interface SelectAutoMobileBandModelCellView()
{
    
}
@property(nonatomic,strong)NSMutableDictionary*carModeldata;
@end
@implementation SelectAutoMobileBandModelCellView
-(void)addFCView{
...
-(void)loadMyDataWithBlock:(AllcallBlock)block{
    ...
    [kCarCenterService welcome_getcar:@{} withBlock:^(id data, int status, NSString *msg) {
...
-(NSString *)valueStr{
    for (NSDictionary*dic in _carModeldata[@"list"]) {
        for (NSDictionary*sdic in dic[@"son"]) {
            if ([[sdic ojsk:@"selected"] isEqualToString:@"1"]) {
                NSMutableDictionary*mdic=[NSMutableDictionary new];
                [mdic setObject:[dic ojsk:@"id"] forKey:@"fid"];
                [mdic setObject:[sdic ojsk:@"id"] forKey:@"sid"];
                return (id)mdic;
            }
        }
    }
...
-(void)setValueStr:(NSString *)valueStr{
    ...
    [self loadMyDataWithBlock:^(id data, int status, NSString *msg) {
        for (NSMutableDictionary*dic in weakSelf.carModeldata[@"list"]) {
            for (NSMutableDictionary*sdic in dic[@"son"]) {
                if ([[sdic ojsk:@"id"] isEqualToString:[mdic ojsk:@"sid"]]) {
...
-(void)selectBtnClick:(UIView*)btn{
    ...
    __weak __typeof(self) weakSelf = self;
    [self loadMyDataWithBlock:^(id data, int status, NSString *msg) {
        [UTILITY.currentViewController pushController:[AutomobileBrandModelViewController class] withInfo:weakSelf.carModeldata withTitle:kST(@"chooseAutomobileBrandModel") withOther:nil withAllBlock:^(id data, int status, NSString *msg) {
            weakSelf.carModeldata=data;
            [weakSelf updataTextfeildInfo];
        }];
    }];
...
-(void)updataTextfeildInfo{
    ...
    for (NSDictionary*dic in _carModeldata[@"list"]) {
        for (NSDictionary*sdic in dic[@"son"]) {
            if ([[sdic ojsk:@"selected"] isEqualToString:@"1"]) {
                self.defaultTextfield.text=[sdic ojsk:@"name"];
                break;
            }
        }
    }
