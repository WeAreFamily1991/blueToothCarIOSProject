...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addView];
    [self navbarTitle:kST(@"modifyPassword")];
}
...
-(void)addView{
    ...
    NSArray*arraytitle=@[kS(@"modifyPassword", @"register_hint_pls_input_mail"),kS(@"modifyPassword", @"register_hint_pls_input_verify_code"),kS(@"modifyPassword", @"forget_pwd_hint_pls_input_new_pwd")];
    for (int i=0; i<arraytitle.count; i++) {
        ...
        if ([arraytitle[i] isEqualToString:kS(@"modifyPassword", @"register_hint_pls_input_verify_code")]) {
            tfText.frameWidth=tfText.frameWidth-80;
            WSSizeButton*btnVerificationCode=[RHMethods buttonWithframe:CGRectMake(tfText.frameXW, 0, 80,  15+13+15) backgroundColor:nil text:kS(@"modifyPassword", @"register_button_send_verify_code") font:14 textColor:rgb(13, 107, 154) radius:0 superview:viewContent];
    ...
    {
        WSSizeButton*btnConfirm=[RHMethods buttonWithframe:CGRectMake(15, 40, kScreenWidth-30, 44) backgroundColor:rgb(13, 107, 154) text:kS(@"modifyPassword", @"button_submit") font:16 textColor:rgb(255, 255, 255) radius:5 superview:nil];
...
-(void)loadSendCode:(NSString *)strCid{
    ...
    [dict setValue:@"2" forKey:@"types"];
    [dict setValue:@"changepwd" forKey:@"type"];
