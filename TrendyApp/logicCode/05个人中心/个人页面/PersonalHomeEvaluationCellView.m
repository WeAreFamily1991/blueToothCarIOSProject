//
//  PersonalHomeEvaluationCellView.m
//  TrendyApp
//
...
@implementation PersonalHomeEvaluationCellView
-(void)initOrReframeView{
    ...
    UILabel*lbDescribe=[RHMethods lableX:15 Y:imgVIcon.frameYH+15 W:kScreenWidth-30 Height:0 font:14 superview:self withColor:rgb(102, 102, 102) text:@"車子準時歸還，歸還後車子已然乾淨整潔車子準時歸還，歸還後車子已然乾淨整潔車子準時歸還，歸還後車子已然乾淨整潔車子準時歸還，歸還後車子已然乾淨整潔" reuseId:@"lbDescribe"];
...
//        UIImageView*imgVPic=[RHMethods imageviewWithFrame:CGRectMake(15+i%3*(width+3), lbDescribe.frameYH+20+i/3*(width+10), width, width) defaultimage:@"photo" supView:self reuseId:[NSString stringWithFormat:@"imgVPic%d",i]];
...
//    }
    ...
    OrderHomeSmallCellView*viewCarInfo=[OrderHomeSmallCellView viewWithFrame:CGRectMake(15, self.frameHeight+15, kScreenWidth-30, 0) backgroundcolor:rgbwhiteColor superView:self reuseId:@"viewCarInfo"];
    viewCarInfo.layer.borderColor=rgb(238, 238, 238).CGColor;
    viewCarInfo.layer.borderWidth=1;
    viewCarInfo.layer.cornerRadius=5;
    viewCarInfo.type=@"personalHome";
    ...
    if (self.isfirstInit) {
        ...
        [self setAddUpdataBlock:^(id data, id weakme) {
            ...
            [viewCarInfo upDataMeWithData:[data ojk:@"order"]];
