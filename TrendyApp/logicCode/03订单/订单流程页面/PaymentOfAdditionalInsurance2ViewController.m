...
@interface PaymentOfAdditionalInsurance2ViewController ()
{
...
@implementation  PaymentOfAdditionalInsurance2ViewController
#pragma mark  bigen
- (void)viewDidLoad {
    [super viewDidLoad];
    //    [self addView];
...
-(void)addView{
    ...
    NSArray*titleArray=@[
                         kS(@"order_detail", @"price_item_car"),
                         kS(@"pay_for_append_insurance", @"pay_insurance_amount"),
                         kS(@"order_detail", @"price_item_favorable_price"),
                         [NSString stringWithFormat:@"%@￥%@",kS(@"pay_for_append_insurance", @"pay_for_insurance_total"),[self.data ojsk:@"totalprice"]]];
    for (int i=0; i<titleArray.count; i++) {
        if (i==2&&[self.data ojsk:@"preferentialprice"].noformPriceStr.floatValue==0) {
            continue;
        }
        ...
        if (i==3) {
            ...
            lbDes.text=[NSString stringWithFormat:@"-￥%@",[self.data ojsk:@"preferentialprice"]];
        }else if(i==1){
            lbDes.text=[NSString stringWithFormat:@"￥%@",[self.data ojsk:@"insurerprice"]];
        }else
        {
            lbDes.text=[NSString stringWithFormat:@"￥%@",[self.data ojsk:@"rentprice"]];
        }
    }
...
//            float price=[weakSelf.data ojsk:@"totalprice"].floatValue;
...
-(void)payBtnClick:(UIButton*)btn{
    if (![_isSelectPolicy isEqualToString:@"1"]) {
        [SVProgressHUD showImage:nil status:[NSString stringWithFormat:@"%@%@",kS(@"confirm_booking_info", @"pls_read_confirm"),kS(@"pay_for_append_insurance", @"pay_license")]];
        return;
    }
