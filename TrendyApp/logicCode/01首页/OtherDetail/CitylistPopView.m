...
#import "CitylistPopView.h"
#import "MYRHTableView.h"
#import "WSButtonGroup.h"
@interface CitylistPopView ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation  CitylistPopView
    ...
    if (self.isfirstInit) {
        ...
        [weakSelf setAddUpdataBlock:^(id data, id weakme) {
            ...
            for (int i=0; i<[data[@"list"] count]; i++) {
                ...
                for (int j=0; j<[mdic[@"addresses"] count]; j++) {
                    
                    if ([[mdic[@"addresses"][j] ojsk:@"selected"] isEqualToString:@"1"]) {
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSInteger numrow = [[[self.data ojk:@"list"][_btnGroup.currentindex] ojk:@"addresses"] count];
    return numrow;
}
...
//            [btnSelect setImageStr:nil SelectImageStr:@"complete1"];
            ...
            [cell setAddUpdataBlock:^(id data, id weakme) {
                [btnSelect setTitle:[data ojsk:@"address"] forState:UIControlStateNormal];
                btnSelect.selected=[[data ojsk:@"selected"] isEqualToString:@"1"];
            }];
        }
        
        NSMutableDictionary*mdic=[[self.data ojk:@"list"][_btnGroup.currentindex] ojk:@"addresses"][indexPath.row];
...
-(void)selectAtSection:(int)section atRow:(int) row{
    ...
    _currrentSelectDic=self.data[@"list"][_btnGroup.currentindex][@"addresses"][row];
