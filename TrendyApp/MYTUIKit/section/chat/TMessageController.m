...
- (NSMutableArray *)transUIMsgFromIMMsg:(NSArray *)msgs
{
    ...
    NSMutableArray*usermarray=[NSMutableArray new];
    NSMutableArray*userIdmarray=[NSMutableArray new];
        ...
        for (int i = 0; i < msg.elemCount; ++i) {
            ...
            if(data){
                ...
                if([[msg getConversation] getType] == TIM_GROUP){
                    ...
                    TIMUserProfile*objx=[msg getSenderProfile];
                    data.name = objx.nickname;
                    data.head=objx.faceURL;
//                    [data setAddValue:objx.faceURL forKey:@"faceURL"] ;
//                    if (msg.sender) {
//                        [usermarray addObject:data];
//                        [userIdmarray addObject:msg.sender];
//                    }else{
//
//                    }
    ...
    [[TIMFriendshipManager sharedInstance] getUsersProfile:userIdmarray forceUpdate:NO succ:^(NSArray<TIMUserProfile *> *profiles) {
        
        for (int i=0; i<profiles.count; i++) {
            TIMUserProfile*user=profiles[i];
            TMessageCellData *data=usermarray[i];
            data.name=user.nickname;
            data.head =user.faceURL;
            if (user.faceURL) {
                [data setAddValue:user.faceURL forKey:@"faceUrl"];
            }
        }
...
- (void)sendMessage:(TMessageCellData *)msg
{
    ...
    msg.name=kUtility_Login.userNickName;
    if (kUtility_Login.userLogo) {
        
        msg.head=kUtility_Login.userLogo;
        
        [msg setAddValue:msg.head forKey:@"faceUrl"];
    }
...
- (NSString *)getDateDisplayString:(NSDate *)date{
    ...
    NSDateFormatter *dateFmt = [[NSDateFormatter alloc ] init ];
         NSTimeZone* GTMzone = [NSTimeZone timeZoneForSecondsFromGMT:9*3600];
         [dateFmt setTimeZone:GTMzone];
//#pragma mark 日期
