 ...
 *  @return 字典
 */
- (NSMutableDictionary *)wkeyValues
{
 ...
 *  @return 字典数组
 */
+ (NSMutableArray *)wkeyValuesArrayWithObjectArray:(NSArray *)objectArray
{
    // 0.判断真实性
    if (![objectArray isKindOfClass:[NSArray class]]) {
        [NSException raise:@"objectArray is not a NSArray" format:@""];
    }
 ...
 *  @return 模型数组
 */
+ (NSMutableArray *)wobjectArrayWithKeyValuesArray:(NSArray *)keyValuesArray
{
    // 1.判断真实性
    if (![keyValuesArray isKindOfClass:[NSArray class]]) {
        [NSException raise:@"keyValuesArray is not a NSArray" format:@""];
    }
