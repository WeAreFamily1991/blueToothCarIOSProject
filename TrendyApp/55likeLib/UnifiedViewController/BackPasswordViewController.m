...
@property (nonatomic, assign)BOOL boolBack;
@property (nonatomic, assign)BOOL boolCode;
...
- (void)initComponents{
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kVersion7?64:44, kScreenWidth,kContentHeight)];
    self.tableView.delegate = (id)self;
    self.tableView.dataSource = (id)self;
    [self.tableView setBackgroundColor:rgbGray];
    [self.tableView setSeparatorStyle:UITableViewCellSeparatorStyleNone];
    [self.view addSubview:self.tableView];
    [self.tableView setAutoresizingMask:UIViewAutoresizingFlexibleHeight];//08-09修改-热点、电话 页面调整
    [self.tableView registerClass:[MoreBlankTableViewHeaderFooterView class] forHeaderFooterViewReuseIdentifier:@"MoreBlankTableViewHeaderFooterView"];
...
-(void)handleKeyboardDidShow:(NSNotification *)notification{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.tableView.frame=CGRectMake(X(self.tableView), Y(self.tableView), W(self.tableView),self.view.frame.size.height- Y(self.tableView)-distanceToMove);
...
{
    [UIView animateWithDuration:0.3 animations:^{        
        self.tableView.frame=CGRectMake(0, Y(self.tableView), kScreenWidth,kContentHeight);
    ...
    switch (indexPath.section) {
        ...
        case 1:
        {
            ...
            if (!cell) {
                ...
                self.btnYZM=[RHMethods buttonWithFrame:CGRectMake(kScreenWidth-70, 7, 60, 30) title:@"发送" image:@"" bgimage:@""];
                self.btnYZM.layer.masksToBounds = YES;
                self.btnYZM.layer.cornerRadius =3.0;
                self.btnYZM.layer.borderWidth=0.5;
                self.btnYZM.layer.borderColor =[[UIColor clearColor] CGColor];
                [self.btnYZM setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
                [self.btnYZM setBackgroundColor:rgbpublicColor];
                [self.btnYZM addTarget:self action:@selector(YZMButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
                [cell addSubview:self.btnYZM];
...
//    }
    ...
    [NetEngine createHttpAction:MFusergetcode withCache:NO withParams:dict withMask:SVProgressHUDMaskTypeNone onCompletion:^(id resData, BOOL isCache) {
        ...
        if ([[resData valueForJSONStrKey:@"status"] isEqualToString:@"200"]) {
            ...
            self.boolBack=YES;
            [SVProgressHUD showSuccessWithStatus:[resData valueForJSONStrKey:@"msg"]];
            [self.btnYZM setBackgroundColor:RGBACOLOR(150, 150, 150, 1)];
...
-(void)updateButtonTitle:(NSString *)str{
    ...
    [self.btnYZM setTitle:[NSString stringWithFormat:@"%.0f秒",aflote] forState:UIControlStateNormal];
    if (aflote<=1) {
...
-(void)yanZM_date{
    DLog(@"延迟——————禁止验证码按钮");
    [self.btnYZM setTitle:@"发送" forState:UIControlStateNormal];
    // [self.btnYZM setTitleColor:rgbpublicColor forState:UIControlStateNormal];
    [self.btnYZM setBackgroundColor:rgbpublicColor];
