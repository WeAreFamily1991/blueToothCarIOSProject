...
- (void)configCell:(LFAssetCell *)cell model:(LFAsset *)model reloadModel:(BOOL)reloadModel
{
    ...
    if (imagePickerVc.showRadioImage) {
        cell.onlyClick=YES;
    } else if (imagePickerVc.maxImagesCount != imagePickerVc.maxVideosCount && model.type == LFAssetMediaTypeVideo) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //单选
    if (imagePickerVc.showRadioImage) {
        LFAsset *model = self.models[index];
        //单选图片裁剪
