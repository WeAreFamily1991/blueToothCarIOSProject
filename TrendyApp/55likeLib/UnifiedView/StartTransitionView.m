...
-(void)loadHeadData{
    ...
    imageTempV.image=[UIImage imageNamed:@"dIcon"];
    ...
    if ([[dic valueForJSONStrKey:@"imageCacheName"] notEmptyOrNull]) {
        [imageTempV imageWithURL:[dic valueForJSONStrKey:@"imageCacheName"]];
...
//        lblsecond.text=[NSString stringWithFormat:@" 跳过广告 %ld ",(long)second];
        lblsecond.text=[NSString stringWithFormat:@" %@ %ld ",kS(@"generalPage", @"skipAdvertise"),(long)second];
...
-(void)APPStartPageADClicked{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{  NSDictionary *dic=[Utility defaultsForKey:default_APPStartPageAD];
        [UTILITY.currentViewController pushController:[SelectWebUrlViewController class] withInfo:@"" withTitle:@" " withOther:@{@"url":[dic valueForJSONStrKey:@"url"]} tabBar:YES pushAdimated:NO];
        [self hiddenGuide];
    });
}
...
//        lblsecond=[RHMethods labelWithFrame:CGRectMake(kScreenWidth-65, kTopHeight-40, 55, 16) font:Font(12) color:[UIColor whiteColor] text:[NSString stringWithFormat:@" 跳过广告 %@ ",[dic valueForJSONStrKey:@"second"]] textAlignment:NSTextAlignmentCenter supView:self];
        lblsecond=[RHMethods labelWithFrame:CGRectMake(kScreenWidth-65, kTopHeight-40, 55, 16) font:Font(12) color:[UIColor whiteColor] text:[NSString stringWithFormat:@" %@ %@ ",kS(@"generalPage", @"skipAdvertise"),[dic valueForJSONStrKey:@"second"]] textAlignment:NSTextAlignmentCenter supView:self];
