...
#define ForegroundStarImage @"starion"
#define BackgroundStarImage @"star0"
...
#pragma mark - block方式
-(instancetype)initWithFrame:(CGRect)frame finish:(completeBlock)finish{
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = 5;
        _rateStyle = WholeStar;
...
-(instancetype)initWithFrame:(CGRect)frame numberOfStars:(NSInteger)numberOfStars rateStyle:(RateStyle)rateStyle isAnination:(BOOL)isAnimation finish:(completeBlock)finish{
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = numberOfStars;
        _rateStyle = rateStyle;
        _isAnimation = isAnimation;
...
+(void)showMyDemo{
    ...
    UILabel * numlabe = [RHMethods labelWithFrame:CGRectMake(0, 200, kScreenWidth, 50) font:Font(18) color:[UIColor redColor] text:@""];
    numlabe.textAlignment = NSTextAlignmentCenter;
    
    XHStarRateView * starview = [[XHStarRateView alloc]initWithFrame:CGRectMake(50, 100, kScreenWidth  - 100, 30) numberOfStars:5 rateStyle:IncompleteStar isAnination:YES finish:^(CGFloat currentScore) {
       
        
        numlabe.text = [NSString stringWithFormat:@"%.2f分",currentScore];
        
        
        
        
    }];
    //只需显示功能就关闭手势 调用一下setCurrentScore方法
//    starview.userInteractionEnabled = NO
    //初始星星个数
    [starview setCurrentScore:2.5];
    
    [UTILITY.currentViewController.view addSubview:numlabe];
    [UTILITY.currentViewController.view addSubview:starview];
