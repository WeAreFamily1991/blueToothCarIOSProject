...
#import "WSImageToolUseListAll.h"

#import "Myimage.h"


@implementation WSImageToolUseListAll
-(void)start{
    //    return;
    //    imageTest
    [self imageWithPath:@"/Users/55like/Desktop/imageTest/testlistall"];
}
...
-(void)imageWithPath:(NSString*)pathStr{
    ...
    for (NSString*spathStr in fillContent) {
        ...
        BOOL isDir = NO;
        [fm fileExistsAtPath:realPath isDirectory:&isDir];
        if (isDir) {
            if ([spathStr rangeOfString:@"code"].length) {
            }else{
                [self imageWithPath:realPath];
            }
...
//    [showsv removeFromSuperview];
    ...
    NSMutableString*importString=[NSMutableString new];
...
//        #import "SelectdateView.h"
        ...
        imagev.frame=CGRectMake(0, showsv.contentHeight+20, imagev.frame.size.width*1, imagev.frame.size.height*1);
...
//                                            [simgess colortongjiSuju];
        ...
        [WSImageToolUseListAll writeFile:[NSString stringWithFormat:@"%@.h",className] data:[[simge writeMeHfileWithClassName:className] dataUsingEncoding:NSUTF8StringEncoding] withPath:firstPath];
        [WSImageToolUseListAll writeFile:[NSString stringWithFormat:@"%@.m",className] data:[[simge writeMeMfileWithClassName:className]  dataUsingEncoding:NSUTF8StringEncoding] withPath:firstPath];
            [WSImageToolUseListAll writeFile:[NSString stringWithFormat:@"%@_small.png",className] data: UIImagePNGRepresentation(imagev.image) withPath:firstPath];
        
    }
    if (marray.count) {
          [WSImageToolUseListAll writeFile:@"001importfile.text" data:[importString  dataUsingEncoding:NSUTF8StringEncoding] withPath:pathStr];
    }
...
+(void)showMyDemo{
    [[WSImageToolUseListAll new]start];
